...
pub fn run(client: &JiraClient, args: &ShowArgs) -> Result<(), AppError> {
    let command = ShowCommand::from_args(args);
...
impl ShowCommand {
    fn from_args(args: &ShowArgs) -> Self {
        Self {
    ...
    fn to_request(&self) -> GetIssueRequest {
        let mut fields = vec![
            "summary".to_string(),
            "status".to_string(),
        ...
        GetIssueRequest {
            issue_id_or_key: self.issue_key.clone(),
            fields,
            expand: Vec::new(),
        }
    }
...
fn execute(client: &JiraClient, command: &ShowCommand) -> Result<ShowIssueOutput, AppError> {
    let response = client
        .get_issue::<ShowIssueFields>(&command.to_request())
...
fn render_human(mut writer: impl Write, issue: &ShowIssueOutput) -> Result<(), AppError> {
...
mod tests {
    ...
    fn show_request_contains_expected_fields_without_comments() {
        let command = ShowCommand::from_args(&ShowArgs {
